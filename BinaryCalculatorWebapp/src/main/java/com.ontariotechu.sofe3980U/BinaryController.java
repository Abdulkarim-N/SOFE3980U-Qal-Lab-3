	...
	@RequestParam(name="operand2", required=false, defaultValue="") String operand2, Model model) {
		...
		switch(operator)
		{
			case "+":
				...
				return "result";
			case "*":
				model.addAttribute("result", Binary.binaryMulti(number1,number2).getValue());
				return "result";
			case "|":
				model.addAttribute("result", Binary.binaryOR(number1,number2).getValue());
				return "result";
			case "&":
				model.addAttribute("result", Binary.binaryAND(number1,number2).getValue());
				return "result";
			default:
				return "Error";
		}
